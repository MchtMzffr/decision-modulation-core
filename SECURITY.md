...
- Private policy implementations (if using `_private/` hook, ensure it's gitignored)
...
- `.env*`, `*.local`, `*.secrets*`
- `runs/`, `traces/`, `*.log`
- `dmc_core/_private/` (private hook directory)

### Redaction

DMC includes redaction utilities in `dmc_core.security` (if present). Trace/audit logs should never include secrets. Test redaction behavior in `tests/test_security_redaction.py` (if exists).
