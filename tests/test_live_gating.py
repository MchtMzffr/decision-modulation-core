...
def test_live_not_allowed_without_mode():
    r = live_execution_allowed(mode="sim", enable_live_flag=True, required_env_vars_present=True)
    assert r.allowed is False
    assert "live" in r.reason.lower() or "mode" in r.reason.lower()


def test_live_not_allowed_without_flag():
    r = live_execution_allowed(mode="live", enable_live_flag=False, required_env_vars_present=True)
    assert r.allowed is False
